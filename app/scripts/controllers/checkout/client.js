    ...
    quotationService
      ...
      .then(function(isValidStock) {
        ...
        if (vm.quotation.Client) {
          clientService
            .getById(vm.quotation.Client.id)
            .then(function(res) {
              vm.client = res.data;
              ...
              if (!vm.isClientFiscalDataValid(vm.client)) {
                dialogService.showDialog(
                  'Los datos fiscales estan incompletos o no son validos'
                );
              }
    ...
    if (vm.quotation.Address && !vm.quotation.immediateDelivery) {
      ...
      if (!selectedContact.Address) {
        dialogService.showDialog('Agrega los datos de envio', function() {
          $location.path('/clients/profile/' + vm.quotation.Client.id).search({
            activeTab: 3,
    ...
    if (vm.quotation.Address || vm.quotation.immediateDelivery || vm.quotation.ShopDelivery || vm.quotation.weekenddate) {
      ...
      quotationService
        ...
        .then(function(goToPayments) {
          ...
          $location
            .path('/checkout/paymentmethod/' + vm.quotation.id)
            .search({});
        })
        .catch(function(err) {
          console.log(err);
