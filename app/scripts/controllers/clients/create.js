'use strict';
angular.module('actualApp').controller('ClientCreateCtrl', ClientCreateCtrl);
...
function ClientCreateCtrl(
  ...
  function validateClientEmails(client) {
    if (client.contacts && client.contacts.length > 0) {
      for (var i = 0; i < client.contacts.length; i++) {
        if (
          !commonService.isValidEmail(client.contacts[i].E_Mail, {
    ...
    if (
      !commonService.isValidEmail(client.E_Mail, { excludeActualDomains: true })
    ) {
      return false;
    }
  ...
  function create(createPersonalForm, createFiscalForm, createDeliveryForm) {
    ...
    var formsRelations = [
      {
        form: createFiscalForm,
        data: vm.client.fiscalAddress,
    ...
    if (
      ...
      clientService
        ...
        .then(function(res) {
          ...
          if (created.CardCode) {
            ...
            if (
              $location.search().continueQuotation ||
              $location.search().checkoutProcess
            ) {
      ...
      if (
        (formsValidationErrors && formsValidationErrors.length > 0) ||
        !createPersonalForm.$valid
      ) {
        var errorTabs = [];
  ...
  function assignClientToQuotation(clientId) {
    ...
    if (activeQuotation.id) {
      quotationService
        .update(activeQuotation.id, params)
        .then(function(res) {
          ...
          if (quotation && quotation.id) {
            ...
            if (
              ...
              $location.path('/clients/profile/' + clientId).search({
                createdClient: true,
